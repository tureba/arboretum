...
void TApp::CreateTree(){
   // create for GH-Tree
   GHTree = new myGHTree(PageManager);
}//end TApp::CreateTree

//------------------------------------------------------------------------------
void TApp::CreateDiskPageManager(){
   //for GHTree
   PageManager = new stPlainDiskPageManager("GHTree.dat", 1024);
}//end TApp::CreateDiskPageManager

//------------------------------------------------------------------------------
void TApp::Run(){
   // Lets load the tree with a lot values from the file.
   cout << "\n\nAdding objects in the GHTree";
   LoadTree(CITYFILE);
...
void TApp::LoadTree(char * fileName){
   ...
   if (GHTree!=NULL){
      if (in.is_open()){
         ...
         cout << " Added " << GHTree->GetNumberOfObjects() << " objects ";
         in.close();
      }else{
...
void TApp::PerformQueries(){
   if (GHTree){
      cout << "\nStarting Statistics for Range Query with GHTree.... ";
      PerformRangeQuery();
      cout << " Ok\n";

      cout << "\nStarting Statistics for Nearest Query with GHTree.... ";
      PerformNearestQuery();
      cout << " Ok\n";
   }//end if
...
void TApp::PerformRangeQuery(){
   ...
   if (GHTree){
      ...
      GHTree->GetMetricEvaluator()->ResetStatistics();
      start = clock();
      for (i = 0; i < size; i++){
         result = GHTree->RangeQuery(queryObjects[i], 0.2);
         delete result;
      }//end for
      end = clock();
      ...
      cout << "\nAvg Distance Calculations: " <<
         (double )GHTree->GetMetricEvaluator()->GetDistanceCount() / (double )size;
   }//end if
...
void TApp::PerformNearestQuery(){
   ...
   if (GHTree){
      ...
      start = clock();
      for (i = 0; i < size; i++){
         result = GHTree->NearestQuery(queryObjects[i], 15);
         delete result;
      }//end for
      end = clock();
      cout << "\nTotal Time: " << ((double )end-(double )start) / 1000.0 << "(s)";
